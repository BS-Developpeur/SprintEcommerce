...
// ------------------------------------
  // SUPPPRIMER LES PRODUITS QUI NE SONT PAS COMMANDES      
...
if ($confirme !== "") {
    ...
    if ($confirme === "OUI") {
        $id = $_POST['produit_id'];
        $codRet = sqlSupprimerProduit($conn, $id);
...
<head>
    <meta charset="UTF-8">
    <meta name="description" content="Liste supprimer produits">
...
<?php if (isset($row)) : ?>
    <?php if (count($row) > 0) : ?>
        <section>
            <p>Confirmez la suppression du produit <?php echo $row['produit_libelle'] ?>  </p>
            <form class="form-suppression" action="supprimerProduit.php" method="post"> 
