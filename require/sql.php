 ...
 */
function sqlModifierCommande($conn,$commande_id,array $produit,  $quantite) {
 
 for($i=0;$i<count($produit);$i++ ) {
      
    $req="UPDATE produits_commandes 
SET
fk_produit_id ='$produit[$i]',
quantite_commande='$quantite'
WHERE fk_commande_id ='$commande_id'";
...
function listesCommandes($conn)
{
    ...
    C.commande_id as 'Numéro de commande',
    CONCAT(CL.client_prenom, ' ', CL.client_nom) as 'Nom du client',
    concat(CL.client_rue,' ',CL.client_ville,' ',CL.client_cp) as 'adresse',
    P.produit_libelle as 'Produit',
    PC.quantite_commande as 'Quantité',
    C.commande_date as 'date',
        ...
        if ($nbResult) {
            ...
            while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                if ($commande_id !== $row['Numéro de commande']) {
                    ...
                    $commande_id = $row['Numéro de commande'];
                    $commande_client = $row['Nom du client'];
                    $commande_etat = $row['date'];
                     $commande_date = $row['etat'];
...
function sqlSupprimerCommandes($conn,$id) {
    ...
    $listeC=listesCommandes($conn);
    
    foreach( $listeC  as $l){
        if($l['commande_id']==$id){
            
             $req ="DELETE FROM produits_commandes WHERE fk_commande_id=".$id;
...
/**
 * Fonction sqlListeClient
 * Auteur : yapo
 * Date   : 
 * But    : Récupérer la list des client
 * Arguments en entrée : $conn = contexte de connexion
 *                      
 * Valeurs de retour   : $row  = ligne correspondant à la clé primaire
 *                               tableau vide si non trouvée 
                               sous forme de tableau associatif
 */
function sqlListeClient($conn, $id) {
...
/**
 * Fonction sqlListeCategorie
 * Auteur : yapo
 * Date   : 
 * But    : Récupérer la categorie 
 * Arguments en entrée : $conn = contexte de connexion
 *                       $id   = clé primaire
 * Valeurs de retour   : $row  = ligne correspondant à la clé primaire
 *                               tableau vide si non trouvée 
                               sous forme de tableau associatif
 */
function sqlListeCategorie($conn) {
...
function sqlSupprimerCategorie($conn, $id) {
    ...
    $listC = sqlListeProduits($conn);

    foreach($listC as $c){
        
      // if($c["fk_categorie_id"] == $id ){      
          $req="UPDATE produits 
                  SET
                  fk_categorie_id =NULL
 ...
 *                               tableau vide si non trouvée 
                               sous forme de tableau associatif
 */
function sqlListeProduits($conn) {
 ...
 * Auteur : yapo
 * Date   : 
 * But    : supprimer une ligne de la table produits  les produit qui n ont pas de categorie
 * Arguments en entrée : $conn = contexte de connexion
 *                       $id   = valeur de la clé primaire 
 * Valeurs de retour   : 1    si suppression effectuée
 *                       0    si aucune suppression
